...
import { WebContent, IWebContent } from '../models';

export class WebContentService {
  /**
   * Get web content for a specific admin or create default content if it doesn't exist
   */
  static async getAdminWebContent(adminUsername: string): Promise<IWebContent> {
    await connectToDatabase();
    
    // Try to find existing content for this admin
    let content = await WebContent.findOne({ adminUsername });
    
    // If no content exists, create default content
    if (!content) {
      content = await this.createDefaultContent(adminUsername);
    }
  ...
  /**
   * Create default web content for a new admin
   */
  static async createDefaultContent(adminUsername: string): Promise<IWebContent> {
    await connectToDatabase();
    
    const defaultContent = new WebContent({
      adminUsername,
   ...
   */
  static async updateWebContent(adminUsername: string, contentData: Partial<IWebContent>): Promise<IWebContent> {
    await connectToDatabase();
  ...
  /**
   * Update a specific section of website content
   */
