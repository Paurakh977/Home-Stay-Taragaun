...
export default function About() {
  ...
  // Default content for initial view or fallback
  ...
  // Use content from database, falling back to defaults for missing parts
  const aboutContent = loading || !content || !content.aboutPage ? defaultContent : {
    hero: {
      title: content.aboutPage?.hero?.title || defaultContent.hero.title,
      subtitle: content.aboutPage?.hero?.subtitle || defaultContent.hero.subtitle,
      backgroundImage: content.aboutPage?.hero?.backgroundImage || defaultContent.hero.backgroundImage
    },
    story: {
      title: content.aboutPage?.story?.title || defaultContent.story.title,
      content: content.aboutPage?.story?.content || defaultContent.story.content,
      imagePath: content.aboutPage?.story?.imagePath || defaultContent.story.imagePath
    },
    values: {
      title: content.aboutPage?.values?.title || defaultContent.values.title,
      subtitle: content.aboutPage?.values?.subtitle || defaultContent.values.subtitle,
      items: Array.isArray(content.aboutPage?.values?.items) && content.aboutPage.values.items.length > 0 
        ? content.aboutPage.values.items 
        : defaultContent.values.items
    },
    team: {
      title: content.aboutPage?.team?.title || defaultContent.team.title,
      subtitle: content.aboutPage?.team?.subtitle || defaultContent.team.subtitle,
      members: Array.isArray(content.aboutPage?.team?.members) && content.aboutPage.team.members.length > 0 
        ? content.aboutPage.team.members 
        : defaultContent.team.members
    },
    offerings: {
      title: content.aboutPage?.offerings?.title || defaultContent.offerings.title,
      subtitle: content.aboutPage?.offerings?.subtitle || defaultContent.offerings.subtitle,
      features: Array.isArray(content.aboutPage?.offerings?.features) && content.aboutPage.offerings.features.length > 0 
        ? content.aboutPage.offerings.features 
        : defaultContent.offerings.features
    },
    impact: {
      title: content.aboutPage?.impact?.title || defaultContent.impact.title,
      content: content.aboutPage?.impact?.content || defaultContent.impact.content,
      stats: Array.isArray(content.aboutPage?.impact?.stats) && content.aboutPage.impact.stats.length > 0 
        ? content.aboutPage.impact.stats 
        : defaultContent.impact.stats,
      imagePath: content.aboutPage?.impact?.imagePath || defaultContent.impact.imagePath
    },
    mission: {
      statement: content.aboutPage?.mission?.statement || defaultContent.mission.statement
    },
    cta: {
      title: content.aboutPage?.cta?.title || defaultContent.cta.title,
      subtitle: content.aboutPage?.cta?.subtitle || defaultContent.cta.subtitle,
      primaryButton: {
        text: content.aboutPage?.cta?.primaryButton?.text || defaultContent.cta.primaryButton.text,
        link: content.aboutPage?.cta?.primaryButton?.link || defaultContent.cta.primaryButton.link
      },
      secondaryButton: {
        text: content.aboutPage?.cta?.secondaryButton?.text || defaultContent.cta.secondaryButton.text,
        link: content.aboutPage?.cta?.secondaryButton?.link || defaultContent.cta.secondaryButton.link
      }
    }
  };
  
  // Sort team members by order property
  const teamMembers = [...aboutContent.team.members].sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
    ...
    <div className="bg-white">
      ...
      <section className="py-24 bg-gray-50 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          <Image
            src={getImageUrl(aboutContent.hero.backgroundImage)}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="relative h-80 md:h-full min-h-[400px] rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <Image
                src={getImageUrl(aboutContent.story.imagePath)}
                alt="Nepal StayLink Story"
                fill
                className="object-cover"
                unoptimized={true}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative h-[450px] rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <Image
                src={getImageUrl(aboutContent.impact.imagePath)}
