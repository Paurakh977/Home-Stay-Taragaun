...
// Sidebar content component that can use chat context
function DashboardSidebarContent({
  ...
  toggleSidebar: () => void;
  handleLogout: () => void;
  renderProfileImage: (size: "small" | "medium") => JSX.Element;
}) {
  ...
  return (
    <>
      ...
      <nav className={`mt-6 flex-1 ${isCollapsed ? "px-2" : "px-4"} space-y-2`}>
        ...
        <Link
          href={`${basePath}/chat`}
          className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} px-4 py-3 rounded-md transition-colors ${isActive("/chat")} ${isCollapsed ? "relative" : ""}`}
