...
export default function AdminDocumentsPage() {
  ...
  // Verify documents access permission
  useEffect(() => {
    const verifyAccess = async () => {
      try {
        ...
        if (!data.homestay?.featureAccess?.documents) {
          toast.error("Access to documents is not permitted");
          router.push(`/${adminUsername}/dashboard`);
          return;
        }
  ...
  // Don't render the documents page until access is verified
  if (!verified) {
    return null;
  }
