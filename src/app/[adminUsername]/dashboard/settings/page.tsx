...
export default function AdminSettingsPage() {
  ...
  // Verify settings access permission
  useEffect(() => {
    const verifyAccess = async () => {
      try {
        ...
        if (!data.homestay?.featureAccess?.settings) {
          toast.error("Access to settings is not permitted");
          router.push(`/${adminUsername}/dashboard`);
          return;
        }
  ...
  // Don't render the settings page until access is verified
  if (!verified) {
    return null;
  }
